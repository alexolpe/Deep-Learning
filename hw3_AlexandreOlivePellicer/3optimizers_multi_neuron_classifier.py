    ...
    ##END MODIFIED CODE---------------------------------------------
      ...
      def backprop_and_update_params_multi_neuron_model(self, predictions, y_errors):
        ...
        for param in partial_of_loss_wrt_params: 
            #We keep the consistency of using a negative value for the partial_of_loss_wrt_param as given in the initial code
            #Apply SGD+ formulas
        ...
        for layer_index in range(1,self.num_layers):           
            for k in range(self.layers_config[layer_index]):
                #We keep the consistency of using a negative value for the partial_of_loss_wrt_param as given in the initial code
                #Apply SGD+ formulas
                bias_v_t_plus_1 = self.u * self.bias_v_t[layer_index][k] - ( bias_changes[layer_index][k] / float(self.batch_size) )
...
SGD_plus_loss = cgp_SGDPlus.run_training_loop_multi_neuron_model( training_data_cgp_SGDPlus )
...
SGD_adam_loss = cgp_Adam.run_training_loop_multi_neuron_model( training_data_cgp_Adam )
